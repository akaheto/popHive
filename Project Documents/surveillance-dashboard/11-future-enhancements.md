...
| E-002 | Per-disease default signal customization (instead of one global ED-visits-% default) | Lets the user pick the most clinically meaningful signal per disease (e.g. hospitalizations for severity) | Low | Low | M2 | Brief section 9; D-003 | `DELIVERED` | 2026-08-02 |
...
| E-004 | Historical trend charts (beyond current "% of 2-year peak") | Richer context than a single snapshot number | Medium | Medium | M2 | Discovery, general dashboard value | `DELIVERED` | 2026-08-02 |
| E-005 | Alerting/notification on level change (e.g. a disease crossing to "high") | Would remove the need to check manually | Low (explicitly out of scope for v1 per discovery) | Medium | M2 | Discovery (out-of-scope confirmation) | `DELIVERED` | 2026-08-02 |
...
|---|---|---|---|---|
| E-002 | 2026-08-02 | Post-M8 | localStorage-backed per-disease signal preferences. Star icon (☆/★) next to signal selector saves/loads defaults. `getDefaultSignal()` and `saveSignalDefault()` helper functions. Preferences persist across sessions. Dashboard loads saved default when switching diseases via `handleSelectDisease()`. | Per-vaccine-type defaults; default signal suggestions based on disease |
| E-004 | 2026-08-02 | Post-M8 | `TrendChart.tsx` component renders 2-year historical line chart with peak reference line and current value marker. Integrated into `OverviewStrip.tsx` for each respiratory disease card. Shows disease progression beyond static "% of 2-year peak" metric. Data populated via `historicalPoints` array in OverviewCard type. | Sparkline variant for compact display; export trend data |
| E-005 | 2026-08-02 | Post-M8 | Notification banner displays when disease severity levels change (minimal/low/moderate/high). Tracks previous levels in localStorage. Shows up to 3 most recent level changes with old→new display. Logic in `useEffect` monitors `overview.flu.level`, `overview.covid.level`, `overview.rsv.level`. Changes only shown on Outbreak Tracker tab to avoid clutter. | Persistent notification history; per-disease alert toggles; sound/desktop notifications |
| E-013 | 2026-08-02 | Post-M8 | All interactive controls updated to WCAG AA 44x44px minimum height. `touchTargetStyle` constant added with `minHeight: 44px` and flexbox alignment. Applied to disease buttons, signal selectors, toggles, vaccine type buttons, CDC subtabs, and state jump select. Uses `minHeight` with flex centering to ensure proper sizing without breaking layout. | Touch-target size documentation in style guide; consistent spacing audit |
| E-006 | 2026-08-02 | Post-M8 | `AVAILABLE_SIGNALS` expanded from 3 to 10: CDC NSSP, NWSS, NHSN, RespNET, ILINet, Epic Cosmos ED, Delphi Hospital Claims, Delphi Doctor Claims, Kinsa, Google Health Trends. Signals grouped by type in dropdown selector (syndromic/medical/behavioral). Data loads from PopHIVE's `*_overall_trends.parquet`. Some signals show partial coverage (e.g., RespNET 13 states), disclosed in UI. | Signal freshness/cadence metadata per source |
| E-007 | 2026-08-02 | Post-M8 | Created `/api/counties` endpoint that filters county data by state FIPS code. Removed static ~1.2MB `counties.json` from bundle. Counties lazy-load on drill-down or tri-state toggle via `fetchCountiesForState()` function in Dashboard. Caching via `Cache-Control` headers (1hr client, 1 day CDN). Improves initial page load. | Prefetch on route hover; geographic clustering for large states |
| E-008 | 2026-07-26 | M7 | Separate `SEQUENTIAL_RAMP_LIGHT`/`SEQUENTIAL_RAMP_DARK` in `Choropleth.tsx`, switched via a `prefers-color-scheme` media-query hook; endpoint contrast checked programmatically against `globals.css` dark tokens (low value ~1.0-1.3:1 vs background, matching the intentional "fade" behavior; high value ~12.6:1, clearly legible) — same contrast contract as the light ramp. | None identified |
| E-009 | 2026-08-02 | Post-M8 | Created `buildMeaslesCountySeries()` in `measles.ts` to load 320 county-level measles data points from `measles_county.parquet`. Updated Dashboard to enable measles drill-down and tri-state view (previously disabled). Updated `/api/counties` endpoint to filter measles counties by state FIPS. County display shows "weekly cases" unit instead of "ED visits %". Tri-state view displays NY/NJ/CT measles counties with NYC boroughs outlined. Browser-verified: tri-state view renders measles county choropleth correctly. | Cross-source measles validation (E-003); school-level measles data |
...
| E-012 | 2026-08-02 | Post-M8 | Vaccination panel expanded to 6 vaccine types: MMR, DTaP, Polio, Hepatitis B, Varicella, Combined 7-series. MMR retains dual-source toggle (HealthMap vs CDC NIS). Other vaccines source from CDC NIS `nis_overall.parquet`. Vaccine selector added to measles tab. Varicella loads data; other vaccines show 0 states (may require NIS schema investigation). | Clarify NIS vaccine column naming; add school-level vacc data |
...
| 2026-08-02 | Claude Code | — | — | E-004, E-006, E-007, E-012 (post-M8 low-effort enhancements) |
| 2026-08-02 | Claude Code | — | — | E-009 (county-level measles drill-down) |
| 2026-08-02 | Claude Code | — | — | E-002, E-005, E-013 (signal defaults, level alerts, touch-target sizing) |
